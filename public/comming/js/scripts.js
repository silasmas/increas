...
(function($) {
  ...
  $("#popup-contact-form").on("submit", function(e) {
    ...
    var sujet = $("#popup_sujet").val();
    var message = $("#popup_message").val();
    var submitButton = form.find('button[type="submit"]'); // Sélectionne le bouton de soumission
    ...
    if (
      ...
      $.ajax({
        ...
        error: function() {
          // Affiche une erreur générale si nécessaire
          $("#popup-form .error")
            .text("Une erreur est survenue. Veuillez réessayer.")
            .fadeIn(500);
        },
        complete: function() {
          // Réactive le bouton de soumission après l'achèvement de la requête
  ...
  $("#contact-form").on("submit", function(e) {
    ...
    var name = $("#cpopup_name").val();
    var email = $("#cpopup_email_address").val();
    var sujet = $("#cpopup_sujet").val();
    var message = $("#cpopup_message").val();
    var submitButton = form.find('button[type="submit"]'); // Sélectionne le bouton de soumission
    ...
    if (
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.reponse) {
            $("#contact-form .csuccess").fadeIn(1000);
            $("#contact-form .cerror").fadeOut(500);
            $(form)[0].reset();
          } else {
          }
        },
  ...
  $("#newsletter-form").on("submit", function(e) {
    ...
    if (isValidEmail(email)) {
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.reponse) {
            $("#newsletter-form .success").fadeIn(1000);
            $("#newsletter-form .error").fadeOut(500);
            $(form)[0].reset();
          } else {
